"""
Overnight training script v2: trains two PURITY models from scratch (random
init), with all loss heads active from epoch 0 and an adaptive learning-rate
schedule.

Differences from train_overnight.py:
  - Random model init (no progressive stages, no checkpoint loading).
  - Full task_weights live from epoch 0, including the new time-spread loss.
  - AdamW (decoupled weight decay) + ReduceLROnPlateau (LR drops automatically
    when training loss plateaus). LR is no longer hand-scheduled per stage.
  - Linear LR warmup over the first N steps to avoid Adam's first-step
    instability when the time-spread loss starts producing big gradients.

Outputs: {weights_dir}/PURITY_{coupling}_v2_epoch{N}.pth, plus a
         _best.pth tracking lowest validation loss.

Usage:
    python train_overnight_v2.py
    python train_overnight_v2.py --epochs 8 --batch_size 16 --only fast3
"""
import argparse
import os
import sys
...
# ---------------------------------------------------------------------------
# Single-stage task weights (from train.ipynb, with the time-spread loss
# active from epoch 0). All losses on; the scheduler handles LR over time.
# ---------------------------------------------------------------------------
TASK_WEIGHTS = {
...
def build_optimizer(model, lr_base, lr_event, lr_bias, weight_decay):
    """Three param groups, AdamW (decoupled weight decay) for all of them.
       - bias scalars  : large LR, no weight decay (they're physical scalars)
       - event-builder : same LR as base in v2 — let scheduler set it
       - everything else: lr_base, weight_decay
    """
    bias_scalar_params = [
    ...
    for i, batch in enumerate(pbar):
        try:
            ...
            if accum_count == accum_steps or is_last_in_epoch:
                # Linear warmup: scales every group's LR by a common factor.
...
def validate(model, dataloader, criterion, task_weights, device, desc,
             max_batches=None):
    model.train()  # match notebook convention; inference_mode handles state
...
def run_one_model(coupling, train_path, val_path, args):
    ...
    # Random-init model (no checkpoint loading).
    ...
    # StepLR: halve LR every `lr_step_size` epochs.
    ...
    print(f"\n  task_weights: {TASK_WEIGHTS}", flush=True)
    print(f"  optimizer LRs (base/event/bias): "
          f"{base_lrs}", flush=True)
    ...
    # Validation loss history file — one line per epoch.
    history_path = os.path.join(args.weights_dir, f"PURITY_{coupling}_v2_loss_history.txt")
    with open(history_path, 'w') as f:
        f.write("epoch\ttrain_loss\tval_loss\n")

    for epoch in range(args.epochs):
        print(f"\n  ===== {coupling} epoch {epoch+1}/{args.epochs} | "
              f"current LRs: {[pg['lr'] for pg in optimizer.param_groups]} =====",
              flush=True)
        ...
        try:
            train_avg, train_breakdown, n_failed, global_step = train_one_epoch(
                ...
                desc=f"{coupling}/ep{epoch+1} train",
            ...
            print(f"\n[abort] training crashed for {coupling} epoch {epoch+1}:", flush=True)
            traceback.print_exc()
            if 'CUDA' in str(e) or 'device-side assert' in str(e):
                raise
            return
        ...
        if val_loader is not None and not args.skip_val:
            val_avg, val_breakdown = validate(
                ...
                desc=f"{coupling}/ep{epoch+1} val",
        ...
        with open(history_path, 'a') as f:
            f.write(f"{epoch+1}\t{train_avg:.6f}\t{val_avg:.6f}\n")

        scheduler.step()

        # Save per-epoch checkpoint
        ep_path = os.path.join(args.weights_dir,
                                f"PURITY_{coupling}_v2_epoch{epoch+1}.pth")
        torch.save({
