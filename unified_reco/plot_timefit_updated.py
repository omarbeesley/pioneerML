...
OUT = "/home/obeesley/pioneerML/unified_reco/updated_plots/02_timefit_highlow.png"
...
CACHE = "/home/obeesley/pioneerML/unified_reco/updated_plots/.timefit_hists_osl.npz"
if os.path.exists(CACHE):
    ...
    pf = sorted(glob.glob(f"{P}/purity_eval/osl_full_pie/shard_*/pie_eval_events.parquet"))
    Ph_h, Pv_h, Ph_l, Pv_l, NPIE = hist_both(pf, True)
    sh = sorted(glob.glob(f"{P}/purity_eval/osl_full_michel/mix_*/pimu_eval_events.parquet"))
...
def draw(ax, r, label, show_legend=True):
    ax.fill_between(ctr, r["flat"], color=COL["acc"], step="mid", label=fr"accidental (sideband-anchored, $\tau$ fit)")
    ax.fill_between(ctr, r["flat"], r["flat"] + r["mic_full"], color=COL["mue"], alpha=0.55, step="mid",
                    label=r"$\pi\to\mu\to e$ (delayed)")
    if r["include_pie"]:
        ax.fill_between(ctr, r["flat"] + r["mic_full"], r["fit_tot"], color=COL["pie"], alpha=0.6, step="mid",
                        label=r"$\pi\to e\nu$ (prompt)")
    ax.plot(ctr, r["fit_tot"], color="k", lw=1.1, label="total fit")
    ax.errorbar(ctr, r["obs"], yerr=r["verr"], fmt="o", ms=2.3, color="k", lw=0.7,
                label=r"data (10M $\pi\to e$ + 33M open michel), osl", zorder=5)
    ax.axvline(r["fitstart"], color="green", ls="--", lw=1.0, label="fit start (stop+5 ns)")
    ax.axvline(0.0, color="k", ls=":", lw=0.7)
    ax.set_yscale("log"); ax.set_xlim(-300, 500)
    pos_obs = r["obs"][r["obs"] > 0]
    ybot = max(pos_obs.min() * 0.5, 1e-2) if pos_obs.size else 1e-2
    ytop = (pos_obs.max() if pos_obs.size else 1.0) * 4.0        # headroom above the peak
    ax.set_ylim(ybot, ytop)
    ax.set_ylabel("weighted events / 2 ns")
    fitstr = (fr"$N_{{\pi e}}={r['Npie']:.1f}\pm{r['sig']:.1f}$ ({r['sig']/r['Npie']*100:.2f}%)"
              if r["include_pie"] else fr"$N_{{\mu}}={r['Nmue']:.0f}$")
    ax.text(0.012, 0.965, f"{label}\n{fitstr}", transform=ax.transAxes, va="top", ha="left",
            fontsize=10, fontweight="bold",
            bbox=dict(boxstyle="round,pad=0.3", fc="white", ec="0.7", alpha=0.9))
    if show_legend:
        ax.legend(fontsize=8, loc="upper right", ncol=2)

fig, (axh, axl) = plt.subplots(2, 1, figsize=(12, 9.6), sharex=True)
draw(axh, R_hi, r"High bin   $E\geq56$ MeV", show_legend=True)
draw(axl, R_lo, r"Low bin   $E<56$ MeV", show_legend=False)
axl.set_xlabel("reconstructed positron time [ns]")
fig.tight_layout()
os.makedirs(os.path.dirname(OUT), exist_ok=True)
fig.savefig(OUT, dpi=140); plt.close(fig)
print(f"wrote {OUT}\n  high: N_pie={R_hi['Npie']:.1f}+-{R_hi['sig']:.1f}  N_mue={R_hi['Nmue']:.0f}  B={R_hi['flat'][0]:.2f}/bin"
      f"\n  low : N_mue={R_lo['Nmue']:.0f}  B={R_lo['flat'][0]:.2f}/bin  (K={K})", flush=True)
